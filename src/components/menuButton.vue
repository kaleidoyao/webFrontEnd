...
export default {
  ...
  methods: {
    showDetail() {
      if(document.querySelector('.list-container').classList.contains('active')) {
    ...
    toLogin() {
      router.push({
        name:"loginPage",query:{id:this.usrid}
      })
    },
    toWrite() {
      router.push({
        name:"writeBlogPage",query:{id:this.usrid}
      })
    },
    toDrifter() {
      router.push({
